...
void Gerenciador_Colisoes::adiocionarEntidade_Colidivel(Entidade_Colidivel* ec)
{
...
bool Gerenciador_Colisoes::estaoColidindo(Entidade_Colidivel* ec1,Entidade_Colidivel* ec2)
{
    ...
    if((tamanho1.x/2+tamanho2.x/2) >= fabs(distancia.x) && (tamanho1.y/2 + tamanho2.y/2 >= fabs(distancia.y)))
        return true;
    return false;
}
